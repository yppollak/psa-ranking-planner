...
const STALE = { rankings: 8 * 24, lists: 72 };   // hours before it is worth saying
...
function renderFreshness(){
  ...
  const f = freshness(p);
  if(f.ranks.source !== "captured" && !f.lists.n){ el.hidden = true; return; }
  ...
  el.innerHTML = parts.join("") + `<span class="fbnote">shared — whoever refreshes, everyone gets it</span>`;
  el.hidden = false;
}
...
function planValue(row, p, basis){
  ...
  if(a.sim) return { value: a.sim.expected, reserve: 0 };
...
const planTight = (a, b) => !planOverlap(a, b)
  && Math.min(Math.abs(a.d - planEnd(b)), Math.abs(b.d - planEnd(a))) <= CLASH_GAP;
...
function planPool(p, from, to, basis){
  ...
  pool(p).forEach(r => {
    ...
    const { value, reserve } = planValue(r, p, basis);
    // A reserve place cannot be planned on, so it stays out of the schedule —
    // but it is not a rejection either, and the tab lists it separately.
    if(value == null){ outside.push(Object.assign({}, r, { reserve })); return; }
    if(listed) measured++; else assumed++;
    out.push(Object.assign({}, r, { value, listed, travel: planTierOf(p, r) }));
  });
  out.outside = outside;
...
function planFill(p, chosen, rank, to, travel, from){
  ...
  for(let guard = 0; guard < PLAN_PASSES; guard++){
    let moved = false;
    // Two sweeps: everything that fits with more than three days' rest, then the
    // rest. With equal-valued candidates this is the only thing standing between
    // you and a year of back-to-back flights.
    for(const tolerate of [false, true]){
      for(const c of rank){
        if(cur.chosen.indexOf(c) >= 0 || cur.chosen.some(x => planOverlap(x, c))) continue;
        if(!tolerate && cur.chosen.some(x => planTight(x, c))) continue;
...
function planOptimise(p, locked, free, to, travel){
  ...
  for(let i = 0; i < chosen.length - 1; i++)
    if(planTight(chosen[i], chosen[i + 1])) tight.push([chosen[i], chosen[i + 1]]);
...
/* ---------- events ---------- */
function bind(){
  loadFieldPrefs(); setupIntros(); watchPin(); bindTips();
