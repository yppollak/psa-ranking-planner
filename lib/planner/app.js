...
// Total points over tournaments played — no worst results dropped. The ranking
// average flatters anyone past 15 events; this is what they have actually done.
const honestAvg = r => (r && r.played > 0 && r.total != null) ? r.total / r.played : null;
...
/* ---------- rendering ---------- */
function render(){ const p = P(); document.body.classList.toggle("no-player", !p); if(!p){ renderPlayerSel(); return; } renderPlayerSel(); loadCapturedRankings(); renderStrip(); ({ planner: renderPlanner, played: renderPlayed, calendar: renderCalendar, settings: renderSettings, points: renderPoints, rankings: renderRankings })[state.tab](); }
...
const stripText = el => el ? el.textContent : "";
function fillPinbar(p, s, c, delta){
  const nums = $("#pinNums"); if(!nums) return;
  const cell = (label, value, cls, vs) =>
    `<span class="m${cls ? " " + cls : ""}"${vs ? ` title="${esc(vs)}"` : ""}><b>${value}</b>${label}</span>`;
  const vsText = (v, ref) => (ref == null || ref === "" || v == null) ? "" : `Card: ${fmtN(Number(ref))}`;
  nums.innerHTML =
    cell("counting", fmtN(s.total), "", vsText(s.total, c.counting)) +
    cell("played", s.played, "", vsText(s.played, c.played)) +
    cell("divisor", s.divisor, "", s.played <= 15 ? "11 until 16 played" : "played − 4") +
    cell("avg", fmtN(s.avg), "", vsText(s.avg, c.average)) +
    cell("rank", s.rank == null ? "—" : "#" + s.rank, "rank", vsText(s.rank, c.rank)) +
    `<span class="who">${esc(p.name)} · ${state.tab === "planner" ? "Planner" : "Played"} · window ${fmtD(s.start)} → ${fmtD(s.end)}</span>`;
  const tabs = $("#pinTabs");
